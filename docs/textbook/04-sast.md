...
A는 `transaction-service`의 송금 처리 코드를 열었다. 금액을 받아 보내는 쪽 잔액에서 빼고 받는 쪽에 더한다. 그게 전부였다. 금액이 양수인지 확인하는 줄은 — 없었다. 음수를 넣으면 뺄셈이 덧셈이 되고, 받는 계좌에서 보내는 계좌로 돈이 역류한다. 인터넷뱅킹에서, 마음만 먹으면 남의 돈을 끌어올 수 있다는 뜻이다.
...
SonarQube의 기본 Quality Gate는 *전체 코드*가 아니라 **새로 바뀐 코드(New Code)**의 등급(`new_security_rating`)을 본다. "이번 변경이 새 위험을 들였는가"를 묻는 것이다. 기존 코드가 아무리 D여도, 이번 커밋이 깨끗하면 게이트는 초록불을 켠다. 사실 합리적인 설계다 — 매번 전체를 막으면 레거시를 떠안은 팀은 아무것도 배포하지 못하니까.
